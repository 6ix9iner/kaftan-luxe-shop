...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    if (videoRef.current) {
      // PROXY REF detected - we must target the injected video element instead
      const injectedVideo = document.getElementById('hero-video') as HTMLVideoElement;

      if (injectedVideo) {
        injectedVideo.defaultMuted = true;
        injectedVideo.muted = true;
        injectedVideo.playsInline = true;

        const playVideo = async () => {
          try {
            await injectedVideo.play();
          } catch (error) {
        ...
        // Handle video end - fade out video to reveal background on all devices
        const handleVideoEnd = () => {
          setVideoEnded(true);
        };

        injectedVideo.addEventListener('ended', handleVideoEnd);

        // Global touch fallback
        const handleTouch = () => {
          if (injectedVideo.paused) {
            injectedVideo.play().catch(e => console.log('Touch kickstart failed', e));
          }
          // Remove listeners once we've tried (but don't remove ended listener)
        ...
        return () => {
          injectedVideo.removeEventListener('ended', handleVideoEnd);
          document.removeEventListener('touchstart', handleTouch);
          document.removeEventListener('click', handleTouch);
        };
      }
  ...
  return <section className="relative h-screen flex items-center justify-center overflow-hidden">
    {/* Video Background */}
    {/* Video Background - Dangerous HTML Injection to bypass React attribute filtering */}
    <div
      className={`absolute inset-0 w-full h-full transition-opacity duration-1000 ${videoEnded ? 'opacity-0' : 'opacity-100'}`}
    ...
    {/* Hidden ref attachment for our useEffect to grab it */}
    <video
      ref={videoRef}
      className="hidden"
      id="hero-video-ref-proxy"
      style={{ display: 'none' }}
    />
